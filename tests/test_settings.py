...
def test_scout_settings_preserve_private_deployment_and_access_token(tmp_path, monkeypatch):
    ...
    (tmp_path / ".env").write_text(
        ...
        "SCOUT_BIND_HOSTS=127.0.0.1,172.28.0.1\n"
        "SCOUT_ALLOWED_HOSTS=localhost,scout.example.com\n"
        "SCOUT_TRUSTED_PROXIES=127.0.0.1,172.28.0.2\n"
    )
    for key in (
        "SCOUT_DATA",
        "SCOUT_API_TOKEN",
    ...
    assert settings.bind_hosts == ("127.0.0.1", "172.28.0.1")
    assert settings.allowed_hosts == ("localhost", "scout.example.com")
    assert settings.trusted_proxies == ("127.0.0.1", "172.28.0.2")
